...
// DIEGO20.03.25:
    public void guardarPedido(JTable jTablePedidos, Integer usuarioDocumento, Integer mesaId) {
    Database objetoConexion = new Database();
    
    try (Connection conexion = objetoConexion.establecerConexion()) {
        DefaultTableModel model = (DefaultTableModel) jTablePedidos.getModel();
        
        // Consulta SQL para insertar en la nueva estructura de la tabla pedido
        String insertSQL = "INSERT INTO pedido (usuario_documento, mesa_idmesa, cantidad, observaciones, fechahora, precio, productomenú_idproducto_menú) VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        try (PreparedStatement ps = conexion.prepareStatement(insertSQL, Statement.RETURN_GENERATED_KEYS)) {
            for (int i = 0; i < model.getRowCount(); i++) {
                try {
                    ...
                    Object obsObj = model.getValueAt(i, 1);
                    Object precioObj = model.getValueAt(i, 2);
                    Object productoObj = model.getValueAt(i, 3);
                    
                    // Convertimos los datos con manejo de errores
                    Integer cantidad = (cantidadObj != null) ? Integer.parseInt(cantidadObj.toString()) : 1;
                    String observaciones = (obsObj != null) ? obsObj.toString() : null;
                    Double precio = (precioObj != null) ? Double.parseDouble(precioObj.toString()) : 0.0;
                    
                    // Verificamos si el valor en la columna 3 es un ID (número) o un nombre de producto
                    Integer productoMenuId;
                    if (productoObj != null) {
                        String productoStr = productoObj.toString();
                        try {
                            // Intentamos convertir directamente a Integer
                            productoMenuId = Integer.parseInt(productoStr);
                        } catch (NumberFormatException e) {
                            // Si no es un número, buscamos el ID del producto por su nombre
                            productoMenuId = buscarIdProductoPorNombre(conexion, productoStr);
                            if (productoMenuId == null) {
                                JOptionPane.showMessageDialog(null, "No se encontró el ID para el producto: " + productoStr);
                                continue; // Skip this row if the product ID is not found
                            }
                            if (productoMenuId == null) {
                                System.out.println("No se encontró ID para el producto: " + productoStr);
                                continue; // Saltamos esta fila
                            }
                        }
                    } else {
                        System.out.println("Valor de producto nulo en la fila " + i);
                        continue; // Saltamos esta fila
                    }
                    
                    // Asignar parámetros a la consulta
                    ps.setObject(1, usuarioDocumento, java.sql.Types.INTEGER);
                    ps.setObject(2, mesaId, java.sql.Types.INTEGER);
                    ps.setInt(3, cantidad);
                    ps.setString(4, observaciones);
                    ps.setTimestamp(5, new java.sql.Timestamp(System.currentTimeMillis()));
                    ps.setDouble(6, precio);
                    ps.setInt(7, productoMenuId);
                    ...
                    System.out.println("Error al procesar la fila " + i + ": " + e.getMessage());
                }
            }
        }
...
// Método auxiliar para buscar el ID de un producto por su nombre
private Integer buscarIdProductoPorNombre(Connection conexion, String nombreProducto) {
    try {
        // Corregido: agregado el esquema y cambiado el nombre de la columna si es necesario
        String sql = "SELECT codigo FROM prueba160225.menu WHERE nombre = ?";
        ...
        System.out.println("Error al buscar ID de producto: " + e.getMessage());
        return null;
    }
}
...
updateStmt.close();
    ...
    public void guardarPedido(JTable tempTable) {
        // Call the three-parameter version with null for usuarioDocumento and mesaId
        // This allows the method to work without those parameters
        guardarPedido(tempTable, null, null);
    }
}
// DIEGO14.03.25: